...
const teachers = [
  ...
  { name: "M. Arifurrahman, M.Hum.", mapel: "Adabul Ilmi", univ: "S2 UIN Sunan Kalijaga", initials: "MA" },
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-foreground leading-tight mb-3">
            Guru <em className="italic text-primary">Profesional</em>
          </h2>
          <div className="w-11 h-[3px] bg-gradient-to-r from-gold to-gold-light rounded-full mx-auto mb-4" />
          <p className="text-muted-foreground text-sm max-w-lg mx-auto">
            Didukung oleh pendidik berkualitas dari universitas terkemuka di Indonesia dan luar negeri
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {teachers.map((t, i) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: Math.min(i * 0.03, 0.4), type: "spring", stiffness: 150, damping: 20 }}
              className="group relative bg-secondary rounded-2xl p-5 text-center shadow-warm hover:shadow-hover hover:-translate-y-1 transition-all duration-500 transition-elegant overflow-hidden"
            >
              ...
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center font-display text-lg font-bold text-primary-foreground mx-auto mb-3">
                {t.initials}
              </div>
              <div className="text-xs font-bold text-foreground leading-tight mb-1">
                {t.name}
              </div>
              <div className="text-[0.68rem] font-semibold text-primary mb-0.5">
                {t.mapel}
              </div>
